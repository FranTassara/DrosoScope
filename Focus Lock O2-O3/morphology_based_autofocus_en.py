...
OPM Autofocus GUI
====================
GUI for real-time autofocus based on the paper
"Active remote focus stabilization in oblique plane microscopy"
Migrated to Pyside

Hardware:
- Camera: Thorlabs Zelux
- Piezo: Thorlabs PFM450 (via Kinesis)

Author: Francisco - Modifications by Tomas
Date: 2026
"""
...
def calculate_robust_moments(image):
    """Calculates center of mass, angle and robust intensity."""
...
def calculate_psf_center(image, roi_size=50):
    """Extracts the ROI and calculates spot parameters."""
...
STYLESHEET = """
/* Palette:
   main bg      #0d0d14   (almost black blue)
   surface      #14141f   (panels, tabs)
   ...
   orange       #ff9e64
   yellow       #e0af68
   violet pos.  #bb9af7   (piezo position)
*/
...
# ============================================================================
# CALIBRATION CURVE DIALOG
# ============================================================================

class CalibrationPlotDialog(QDialog):
    """Temporarily displays the calibration curve after automatic calibration."""
    ...
    def __init__(self, calib_data: dict, parent=None):
        ...
        ax1.scatter(z, np.degrees(ang), s=18, color='#7aa2f7', zorder=3, label='data')
        ax1.plot(z_fit, np.degrees(ang_fit), color='#ff9e64', lw=1.5,
                 label=f'fit  {np.degrees(slope_ang)*1e3:.2f} m°/µm')
        ax1.set_xlabel('Piezo (µm)')
        ax1.set_ylabel('Angle (°)')
        ax1.set_title('Angle vs Position')
        ax1.legend(fontsize=7, facecolor='#252535', labelcolor='#c0caf5', edgecolor='#3b3d5c')

        ax2.scatter(z, ity, s=18, color='#9ece6a', zorder=3, label='data')
        ax2.plot(z_fit, ity_fit, color='#ff9e64', lw=1.5,
                 label=f'fit  {slope_ity:.1f} cts/µm')
...
class HelpDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Operation Guide")
        ...
        <h4>1. Initial Configuration (Setup)</h4>
        <ol>
            <li>Connect hardware: Camera and Piezo. Choose an appropriate working ROI for the camera (ideally centered on the laser spot).</li>
            <li>Enter desired PID gains in the <b>Control Parameters</b> group.</li>
            <li>In the <i>Calibration</i> tab, press the <b>Run Calibration</b> button to establish the system response and initial laser spot sensitivity.</li>
            <li>In the <i>Autofocus</i> tab, press <b>Set Setpoint</b> to define the current spot position as the focus reference (zero point).</li>
        </ol>

        <h4>2. Operation</h4>
        <ol>
            <li>Real-time transmission in the <b>Camera View</b> group provides constant visual feedback of the PSF (Point Spread Function).</li>
            <li>Press the <b>START</b> button to activate the control loop. The system will automatically correct thermal or mechanical drift relative to the setpoint.</li>
        </ol>

        <h4>3. Adjustments and Recalibration</h4>
        <ol>
            <li><b>Parameter update:</b> Press the <code>STOP</code> button → Adjust PID gains → Press the <code>START</code> button.</li>
            <li><b>System recalibration:</b> If necessary, press <code>STOP</code> → Press the <code>RESET</code> button and repeat the <b>Initial Configuration</b> process to establish a new reference position.</li>
        </ol>

        <h3>Data Saving Protocol</h3>
        <ol>
            <li><b>Calibration (JSON/CSV):</b> Generated automatically when a scan finishes. The <code>JSON</code> format is mandatory if you want to reload the response curve in future sessions.</li>
            <li><b>CSV Log:</b> Records the complete history of the autofocus session (ideal for long-term stability analysis).</li>
            <li><b>Error Data:</b> Saves only the last 300 points displayed in the interface graphs.</li>
        </ol>
        """
...
class AutofocusGUI(QMainWindow):
    ...
    # Signals for background thread → UI thread communication
    sig_log = Signal(str)
    sig_saturation = Signal()
    sig_calib_finished = Signal(bool, str)       # (success, final_message)
    sig_calib_data = Signal(object)              # dict with calibration arrays for plotting
    sig_maxint_finished = Signal(bool, float, float, str)  # (success, pos, val, message)

    def __init__(self):
        super().__init__()
        self.setWindowTitle("OPM Autofocus")
        ...
        # Cached interval value: background threads read from here
        # (they never read directly from the widget in another thread)
    ...
    def save_config(self):
        try:
            ...
            self.log_status(f"✓ Configuration saved to {CONFIG_FILENAME}")
    ...
    def _build_left_panel(self, layout):
        ...
        header_layout.addSpacing(24)  # Invisible spacer to compensate for button and center title
    ...
    def _build_hardware_tab(self):
        ...
        self.btn_connect_piezo = QPushButton("Connect Piezo")
        ...
        lim_grid.addWidget(QLabel("Max:"), 1, 0, Qt.AlignRight)
        ...
        self.btn_apply_limits = QPushButton("Apply Limits")
        self.btn_apply_limits.clicked.connect(self.apply_piezo_limits)
        pg.addWidget(self.btn_apply_limits)

        self.limits_status_label = QLabel(f"Range: {self.pos_min_um:.0f} - {self.pos_max_um:.0f} µm")
        ...
        exp_row = QHBoxLayout()
        exp_row.addWidget(QLabel("Exposure (µs):"))
        ...
        self.btn_set_exposure = QPushButton("Apply")
        ...
        self.btn_set_roi = QPushButton("Configure")
        ...
        acq_row = QHBoxLayout()
        acq_row.addWidget(QLabel("Interval (ms):"))
    ...
    def _build_calib_tab(self):
        ...
        # Load calibration
        self.btn_load_calib = QPushButton("Load Calibration (JSON)")
        ...
        # ── PID Error Source ───────────────────────────────────────────────────
        pid_info = QGroupBox("Error Source (PID)")
        pi = QVBoxLayout(pid_info)

        mode_row = QHBoxLayout()
        mode_row.addWidget(QLabel("Source:"))
        self.combo_control_mode = QComboBox()
        self.combo_control_mode.addItem('Angle', 'angle')
        self.combo_control_mode.addItem('Intensity', 'intensity')
        ...
        pi.addWidget(QLabel("(both errors are calculated and saved equally)"))

        warn_lbl = QLabel("[!] Requires prior calibration (auto or manual)")
        ...
        self.lbl_maxint_result = QLabel("Maximum at: --")
        ...
        # ── Manual Sensitivity Input ───────────────────────────────────────────
        manual_group = QGroupBox("Manual Sensitivity Input")
        mng = QGridLayout(manual_group)

        mng.addWidget(QLabel("Angle (rad/µm):"), 0, 0, Qt.AlignRight)
        ...
        self.btn_set_manual_sens = QPushButton("Apply Manual Values")
        ...
        sens_group = QGroupBox("Current Sensitivities")
        sg = QVBoxLayout(sens_group)

        self.lbl_sens_angle = QLabel("Angle: -- rad/µm")
        self.lbl_sens_angle.setFont(QFont("Segoe UI", 10, QFont.Bold))
        sg.addWidget(self.lbl_sens_angle)

        self.lbl_sens_int = QLabel("Intensity: -- cts/µm")
        ...
        scroll.setWidget(container)
        self.tabs.addTab(scroll, "Calibration")
    ...
    def _update_control_mode(self, index):
        ...
        self.log_status(f"PID error source changed to: {self.control_mode}")
    ...
    def _build_autofocus_tab(self):
        ...
        self.btn_set_setpoint = QPushButton("Set Setpoint")
        ...
        self.setpoint_angle_label = QLabel("Angle: -- | Intensity: --")
        ...
        db_group = QGroupBox("Correction Threshold")
        dbg = QVBoxLayout(db_group)

        db_row = QHBoxLayout()
        db_row.addWidget(QLabel("Min. movement (nm):"))
        ...
        db_hint = QLabel("Only moves piezo if |delta| > threshold")
        ...
        # ── Monitoring Only ────────────────────────────────────────────────────
        self.chk_monitor_only = QCheckBox("Monitoring only (do not move piezo)")
        ...
        self.btn_save_log = QPushButton("Save CSV Log")
        self.btn_save_log.clicked.connect(self.save_log)
        save_row.addWidget(self.btn_save_log)

        self.btn_save_error_data = QPushButton("Save Error Data")
        ...
        layout.addStretch()
        scroll.setWidget(container)
        self.tabs.addTab(scroll, "Autofocus")
    ...
    def _build_config_tab(self):
        ...
        self.btn_load_config = QPushButton("Reload Configuration")
        ...
        info = QLabel(
            ...
            "  ← / →  :  Move piezo\n"
            "  Space   :  Set setpoint\n"
            "  S        :  Start / Stop autofocus\n\n"
            "Recommendations:\n"
            "  - 500 ms interval  →  2 Hz  (slow drift)\n"
            "  - 100 ms interval  →  10 Hz (fast drift)\n"
            "  - Small ROI  →  faster speed"
        )
    ...
    def _build_right_panel(self, layout):
        ...
        for ax, title_txt, ylabel_txt in [
            (self.ax1, "Piezo Position", "Position (µm)"),
            (self.ax2, "System Z Error", "Error (µm)"),
            (self.ax3, "PID Error Source", ""),
        ]:
        ...
        self.ax3.set_xlabel("Time (s)", color='#6c7086', fontsize=7)
    ...
    def connect_piezo(self):
        try:
            ...
            self.log_status(f"Connecting to piezo {serial}...")
            ...
            self.log_status("✓ Piezo connected")
            self.update_ui_state()

        except Exception as e:
            self.log_status(f"✗ Error connecting piezo: {e}")
            QMessageBox.critical(self, "Error", f"Could not connect to piezo:\n{e}")

    def connect_camera(self):
        try:
            serial = self.entry_camera_serial.text().strip()
            self.log_status(f"Connecting to camera {serial}...")
            ...
            if serial not in available:
                self.log_status(f"Available cameras: {available}")
            ...
            self.log_status(f"  Resolution: {self.camera.image_width_pixels}x{self.camera.image_height_pixels}")
            self.update_ui_state()

        except Exception as e:
            self.log_status(f"✗ Error connecting camera: {e}")
            QMessageBox.critical(self, "Error", f"Could not connect to camera:\n{e}")
    ...
    def set_piezo_position(self, pos_um):
        ...
        if abs(original - pos_um) > 0.1 and self.running:
            self.log_status(f"⚠ Limit reached: {pos_um:.1f} µm")
    ...
    def move_piezo(self, direction):
        ...
        try:
            ...
            self.log_status(f"Error moving piezo: {e}")
    ...
    def apply_piezo_limits(self):
        try:
            ...
            if min_val >= max_val:
                QMessageBox.critical(self, "Error", "Minimum limit must be less than maximum")
                return

            if min_val < 0 or max_val > 450:
                reply = QMessageBox.question(self, "Warning",
                    f"Limits are outside typical range (0-450 µm).\nContinue?")
                if reply != QMessageBox.Yes:
                    return

            self.pos_min_um = min_val
            self.pos_max_um = max_val
            self.limits_status_label.setText(f"Range: {self.pos_min_um:.0f} - {self.pos_max_um:.0f} µm")
            home_pos = (self.pos_min_um + self.pos_max_um) / 2
            ...
            if self.piezo_connected:
                ...
                if current_pos < self.pos_min_um or current_pos > self.pos_max_um:
                    reply = QMessageBox.question(self, "Position out of range",
                        f"Current position ({current_pos:.1f} µm) is outside new limits.\n"
                        f"Move to center position ({home_pos:.1f} µm)?")
                    if reply == QMessageBox.Yes:
                        self.set_piezo_position(home_pos)

            self.log_status(f"✓ Limits updated: {self.pos_min_um:.0f} - {self.pos_max_um:.0f} µm")

        except ValueError:
            QMessageBox.critical(self, "Error", "Invalid values. Enter valid numbers.")
        except Exception as e:
            self.log_status(f"✗ Error applying limits: {e}")
    ...
    def set_exposure(self):
        ...
        try:
            ...
            self.log_status(f"Exposure → {exp} µs")
        except Exception as e:
    ...
    def configure_roi(self):
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle("Configure ROI")
        dialog.setFixedWidth(280)
        layout = QVBoxLayout(dialog)

        title = QLabel("Configure Region of Interest (ROI)")
        ...
        grid = QGridLayout()
        fields = {}
        for i, (label, key) in enumerate([("X:", 'x'), ("Y:", 'y'), ("Width:", 'width'), ("Height:", 'height')]):
        ...
        def apply_roi():
            try:
                ...
                self.log_status(f"ROI configured: {self.camera_roi}")
                dialog.accept()
            except ValueError:
                QMessageBox.critical(dialog, "Error", "Invalid values")
    ...
    def reset_roi(self):
        ...
        self.update_roi_label()
        self.log_status("ROI reset to full frame")
    ...
    def _update_acq_interval_cache(self, text):
        """Updates the cached value used by background threads."""
        try:
    ...
    def _acquisition_thread(self):
        while self.acquisition_running:
            interval_s = self._acq_interval_s  # read cache, not the widget
            ...
            try:
                ...
                self.sig_log.emit(f"Acquisition error: {e}")
    ...
    def _handle_saturation(self):
        ...
        QMessageBox.warning(self, "Image Saturated!",
            f"Saturation detected (≥ {self.saturation_threshold} cts).\n"
            "Preview paused. Reduce exposure before continuing.")
    ...
    def update_preview(self):
        ...
        try:
            ...
            self.spot_info_label.setText(f"Max Px: {max_val} cts at ({x_bright}, {y_bright})")
    ...
    def load_calibration(self):
        filename, _ = QFileDialog.getOpenFileName(
            self, "Select calibration file", "", "JSON files (*.json)",
        ...
        try:
            ...
            self.update_ui_state()
            self.log_status("✓ JSON calibration loaded successfully.")
            self.log_status(f"  Angle: {self.slope_angle:.5f} rad/µm | Intensity: {self.slope_intensity:.2f} cts/µm")
        except KeyError as e:
            self.log_status(f"✗ Error: incorrect JSON format. Missing: {e}")
            QMessageBox.critical(self, "Format Error", f"The JSON file does not have the expected structure.\nMissing: {e}")
        except Exception as e:
            self.log_status(f"✗ Error loading calibration: {e}")
            QMessageBox.critical(self, "Error", str(e))

    def run_calibration(self):
        if not (self.camera_connected and self.piezo_connected):
            QMessageBox.critical(self, "Error", "Connect all hardware first")
            return
        try:
        ...
        reply = QMessageBox.question(self, "Calibration",
            f"Scan {calib_range} µm in {n_steps} steps.\nContinue?")
        if reply != QMessageBox.Yes:
            return

        self.calibrating = True
        self.btn_run_calib.setEnabled(False)
        self.log_status("Calibrating...")

        # Capture UI values before launching thread
    ...
    def _calibration_thread(self, calib_range, n_steps, interval_s, fmt):
        ...
        try:
            ...
            for i, pos in enumerate(positions):
                ...
                if not got:
                    self.sig_log.emit(f"⚠ Timeout at step {i+1}")
                    continue
                ...
                self.sig_log.emit(f"Calib step: {i+1}/{n_steps}")
            ...
            if len(results) > 2:
                ...
                if fmt == 'CSV':
                    ...
                    try:
                        ...
                        self.sig_calib_finished.emit(False, f"✗ Error saving JSON: {e}")
                        return
                ...
                self.sig_calib_finished.emit(True, f"✓ Calibration saved to {filename}")
            else:
                self.sig_calib_finished.emit(False, "✗ Failed: too few valid points")
    ...
    def _show_calib_plot(self, calib_data: dict):
        ...
        dlg.show()   # non-blocking: user closes when ready
    ...
    def run_max_intensity_search(self):
        ...
        reply = QMessageBox.question(self, "Maximum Search",
            f"Scan {search_range} µm in {n_steps} steps.\nContinue?")
        if reply != QMessageBox.Yes:
            return

        self.btn_run_maxint.setEnabled(False)
        self.lbl_maxint_result.setText("Searching...")
        self.log_status("Searching for maximum intensity...")
    ...
    def _max_intensity_thread(self, search_range, n_steps, interval_s):
        ...
        try:
            ...
            for i, pos in enumerate(positions):
                ...
                self.sig_log.emit(f"Max int step: {i+1}/{n_steps}")
            ...
            if peak_values:
                ...
                self.sig_maxint_finished.emit(True, best_pos, best_val,
                    f"✓ Maximum intensity at {best_pos:.2f} µm ({best_val:.0f} cts)")
            else:
                self.sig_maxint_finished.emit(False, 0.0, 0.0, "✗ No valid data obtained")
    ...
    def _on_maxint_finished(self, success, best_pos, best_val, message):
        self.log_status(message)
        if success:
            self.lbl_maxint_result.setText(f"Maximum at: {best_pos:.2f} µm  ({best_val:.0f} cts)")
        else:
            self.lbl_maxint_result.setText("No valid data")
    ...
    def set_manual_sensitivity(self):
        try:
            ...
            self.update_sensitivity_display()
            self.log_status(f"✓ Manual applied: Angle={self.slope_angle:.5f}, Int={self.slope_intensity:.2f}")
            self.calibrated = True
            self.update_ui_state()
        except ValueError:
    ...
    def update_sensitivity_display(self):
        ...
        self.lbl_sens_angle.setText(f"Angle: {ang_val:.5f} rad/µm")
        self.lbl_sens_int.setText(f"Intensity: {int_val:.2f} cts/µm")
    ...
    def set_setpoint(self):
        if self.current_image is None:
            QMessageBox.critical(self, "Error", "No image in memory. Start preview first.")
            return

        self.log_status("Calculating exact setpoint (PCA)...")
        cx, cy, angle, intensity = calculate_psf_center(self.current_image)

        if cx is None:
            QMessageBox.critical(self, "Error", "Could not detect spot to set setpoint.")
            return
        ...
        self.setpoint_angle_label.setText(f"Angle: {angle_deg:.2f}°  |  Intensity: {intensity:.0f}")
        self.log_status(f"✓ Setpoint set: ({cx:.1f}, {cy:.1f}) | Ang={angle_deg:.2f}° | Int={intensity:.0f}")
        self.btn_start.setEnabled(True)
    ...
    def start_autofocus(self):
        if not self.calibrated:
            QMessageBox.critical(self, "Error", "Set the setpoint first")
            return
        ...
        try:
            ...
            # Cache values for thread (don't read widgets from another thread)
            ...
            self.log_status(f"Starting autofocus..." if not self.chk_monitor_only.isChecked()
                            else "Starting MONITORING (piezo fixed)...")
            self.log_status(f"  PID: Kp={kp}, Ki={ki}, Kd={kd} | Threshold: {af_deadband_um*1000:.0f} nm")
    ...
    def autofocus_loop(self, interval_s, deadband_um):
        while self.running:
            try:
                ...
                self.sig_log.emit(f"Loop error: {e}")
    ...
    def update_plots(self):
        ...
        try:
            if len(self.time_data) > 1:
                ...
                if self.control_mode == 'angle':
                    self.ax3.set_title("Angle Error (raw)", color='#cdd6f4', fontsize=8, pad=4)
                    self.ax3.set_ylabel("Δangle (°)", color='#6c7086', fontsize=7)
                    self.line3a.set_data(t_list, np.degrees(list(self.err_ang_data)))
                    self.line3a.set_label("Δangle (°)")
                else:
                    self.ax3.set_title("Intensity Error (raw)", color='#cdd6f4', fontsize=8, pad=4)
                    self.ax3.set_ylabel("Δintensity (cts)", color='#6c7086', fontsize=7)
                    self.line3a.set_data(t_list, list(self.err_int_data))
                    self.line3a.set_label("Δintensity (cts)")
            ...
            if image is not None:
                ...
                delta_str = ""
                if self.setpoint_angle is not None:
                    delta_str = f"  Δang: {np.degrees(self.current_angle - self.setpoint_angle):+.2f}°"
    ...
    def stop_autofocus(self):
        ...
        self.log_status("Autofocus stopped")
        self.btn_start.setEnabled(True)
        self.btn_stop.setEnabled(False)
    ...
    def save_log(self):
        ...
        if filename:
            ...
            try:
                ...
                self.log_status(f"✗ Error saving log: {e}")
                QMessageBox.critical(self, "Error", f"Could not save log:\n{e}")

    def save_error_data(self):
        if len(self.time_data) == 0:
            QMessageBox.information(self, "Info", "No data in memory to save.")
            return
        filename, _ = QFileDialog.getSaveFileName(
        ...
        if filename:
            try:
                ...
                self.log_status(f"✗ Error saving data: {e}")
                QMessageBox.critical(self, "Error", f"Could not save:\n{e}")

    def reload_config(self):
        self.config = self.load_config()
        self.log_status("Configuration reloaded")
    ...
    def _array_to_pixmap(self, rgb_array):
        """Converts HxWx3 uint8 numpy array to QPixmap."""
